...
class NeuralAgent():
	def __init__(self, env_name: str, problem_name: str, num_timesteps:float, algorithm=SAC, reward_threshold: float=450,
			  	 exploration_rate=None, tasks_to_complete=None, complex_obs_space=False, is_gc=False):
		...
		self._model_directory = get_agent_model_dir(env_name=self.env_name, model_name=problem_name, class_name=algorithm.__name__)
		if os.path.exists(os.path.join(self._model_directory, f"{self.problem_name}.zip")):
			self.pre_trained_model = True
			self._model_file_path = os.path.join(self._model_directory, f"{self.problem_name}.zip")
		else:
	...
	def generate_observation(self, action_selection_method: MethodType, random_optimalism, save_fig=False,
		...
		if save_fig:
			...
			while True:
				...
				try:
					self.record_video(vid_path, desired)
					break
				except Exception as e:
					num_tries += 1
	...
	def generate_partial_observation_gc(self, action_selection_method, percentage, is_fragmented, goal_directed_problem=None, goal_directed_goal=None, save_fig=False, random_optimalism=True):
		assert self.is_gc
		steps = self.generate_observation_gc(action_selection_method, save_fig=save_fig, random_optimalism=random_optimalism, goal_directed_problem=goal_directed_problem, goal_directed_goal=goal_directed_goal) # steps are a full observation
		return random_subset_with_order(steps, (int)(percentage * len(steps)), is_fragmented)

	def generate_observation_gc(self, action_selection_method: MethodType, random_optimalism, goal_directed_problem=None, goal_directed_goal=None,
								save_fig = False, specific_vid_name=None, with_dict=False):
		# print(f"hyperparams:{hyperparams}")
		if goal_directed_problem:
			if save_fig: specific_vid_name = f"gc_{goal_directed_problem}"
			else: specific_vid_name = None
			...
			self.env = create_vec_env(kwargs)
			orig_env = self.env
			observations = self.generate_observation(action_selection_method=action_selection_method, random_optimalism=random_optimalism,
													 save_fig=save_fig, specific_vid_name=specific_vid_name, with_dict=with_dict)
			self.env = orig_env
		else: #goal_directed_goal!=None
			if save_fig: specific_vid_name = f"gc_{goal_directed_goal}"
			else: specific_vid_name = None
			...
			observations = self.generate_observation(action_selection_method=action_selection_method, random_optimalism=random_optimalism,
											save_fig=save_fig, specific_vid_name=specific_vid_name, with_dict=with_dict, desired=goal_directed_goal)
		return observations
