...
from GRAML.recognizer.graml_recognizer import Graml
...
if __name__ == "__main__":
	assert len(sys.argv) == 3, f"Assertion failed: len(sys.argv) is {len(sys.argv)} while it needs to be 3.\n Example: \n\t /usr/bin/python scripts/generate_embeddings_source_tasks.py MiniGrid-Walls-13x13-v0 dataset/MiniGrid-Walls-13x13-v0/models/[11x1]_[11x11]_[1x11]_[8x1]/Graml/base_problems.conf"
	...
	model_directory = get_model_dir(env_name=env_name, model_name=problem_list_to_str_tuple(problem_names), class_name=Graml.__name__)
	model_file_path = os.path.join(model_directory, r'lstm_cnn_model.pth')
	if not os.path.exists(model_file_path):
		raise FileNotFoundError(f"The path {model_file_path} does not exist (perhaps you haven't trained a GRAML model, run graml_main.py)\n Example: \n\t /usr/bin/python scripts/generate_embeddings_source_tasks.py MiniGrid-Walls-13x13-v0 dataset/MiniGrid-Walls-13x13-v0/models/[11x1]_[11x11]_[1x11]_[8x1]/Graml/problems_config.txt")
